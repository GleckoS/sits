...
export default function JobOfferHeader({ data: { title, image, language, city, employmentType, shiftType, createdAt, validUntil, noExpirationDate } }) {
  ...
  return (
    <Header>
      ...
      <div className="dates">
        ...
        <div>
          <span>{applicationTime[language]}:</span> <date>{!noExpirationDate ? formatDate(validUntil) : noValidUntil[language]}</date>
        </div>
      </div>
    </Header>
  );
